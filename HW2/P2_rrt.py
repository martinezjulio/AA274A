    ...
    def solve(self, eps, max_iters=1000, goal_bias=0.05, shortcut=False):
        ...
        for k in range(1,max_iters):
            ...
            if z < goal_bias:
                ...
                x_rand = np.random.uniform(low=self.statespace_lo,high=self.statespace_hi,size=state_dim)
            nearest_index = self.find_nearest(V[:n],x_rand)
            x_near = V[nearest_index]      
            ...
            if len(self.path) > 2:
                ...
                for i in range(1,path_length-1): 
                    ...
                    if self.is_free_motion(self.obstacles, x_parent, x_child):
                        ...
                        x_child = x
                if len(removals) > 0:
                    self.path = np.delete(self.path, removals,axis=0)
        ########## Code ends here ##########
...
class GeometricRRT(RRT):
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        ########## Code starts here ##########
        # Hint: This should take one line.
        state_x1 = np.array(x1)
        state_x2 = np.array(x2)
        distance = np.linalg.norm(state_x1 - state_x2)
        direction_of_x2 = (state_x2 - state_x1)/np.linalg.norm(state_x2 - state_x1)
        return state_x2 if (distance < eps) else (state_x1 + eps*direction_of_x2)
        ########## Code ends here ##########
...
class DubinsRRT(RRT):
    ...
    def find_nearest(self, V, x):
        ...
        dist = []
        for v_i in V:
            path = dubins.shortest_path(v_i, x, self.turning_radius)
            dist.append(path.path_length())
        nearest_index = np.argmin(dist)
        return nearest_index
        ########## Code ends here ##########
    ...
    def steer_towards(self, x1, x2, eps):
        ...
        return x2 if (path.path_length() < eps) else path.sample_many(eps)[0][1]
        ########## Code ends here ##########

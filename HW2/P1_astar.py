...
class AStar(object):
    ...
    def is_free(self, x):
        ...
        for i in range(len(x)):
            if x[i] < self.statespace_hi[i] and x[i] >= self.statespace_lo[i]:
                in_statespace = True
            else:
                in_statespace = False
                break
        ...
        not_inside_any_obstacle = self.occupancy.is_free(x)

        # if neither than is free, otherwise is not
        if in_statespace and not_inside_any_obstacle:
           state_x_is_free = True   
        else:
            state_x_is_free = False

        return state_x_is_free
        ########## Code ends here ##########
    ...
    def distance(self, x1, x2):
        ...
        point1 = np.array(x1)
        point2 = np.array(x2)
        dist = np.linalg.norm(point1 - point2) # or equivalently dist = np.sqrt(np.sum(np.square(point1 - point2)))
        return dist
        ########## Code ends here ##########
    ...
    def get_neighbors(self, x):
        ...
        neighbors = []
        ########## Code starts here ##########
        directions = self.resolution*np.array([[1,0],   # right
                                               [1,1],   # right-up 
                                               [0,1],   # up
                                               [-1,1],  # left-up 
                                               [-1,0],  # left
        ...
        grid_x = np.array(self.snap_to_grid(x_array))
        neighbors = []
        for direction in directions:
            candidate_neighbor = np.array([grid_x[0] + direction[0], grid_x[1] + direction[1]])
            #grid_candidate_neighbor = self.snap_to_grid(candidate_neighbor)
            if self.is_free(candidate_neighbor):
                neighbors.append(self.snap_to_grid(candidate_neighbor))
        ########## Code ends here ##########
        return neighbors

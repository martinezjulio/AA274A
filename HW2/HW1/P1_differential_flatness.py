...
def compute_traj_coeffs(initial_state, final_state, tf):
    ...
    x_init = initial_state.x
    y_init = initial_state.y
    V_init = initial_state.V
    theta_init = initial_state.th
    
    #x_final, y_final, V_final, theta_final = final_state
    x_final = final_state.x
    y_final = final_state.y
    V_final = final_state.V
    theta_final = final_state.th
    
    A1 = np.array([[1, t_init, t_init**2, t_init**3],
                  [0, 1, 2*t_init, 3*(t_init**2)],
                  [1, t_final, t_final**2, t_final**3],
                  [0, 1, 2*t_final, 3*(t_final**2)]])
    b1 = [x_init, V_init*math.cos(theta_init), x_final, V_init*math.cos(theta_final)]
    
    A2 = np.array([[1, t_init, t_init**2, t_init**3],
                  [0, 1, 2*t_init, 3*(t_init**2)],
                  [1, t_final, t_final**2, t_final**3],
                  [0, 1, 2*t_final, 3*(t_final**2)]])
    b2 = [x_init, V_init*math.sin(theta_init), x_final, V_init*math.sin(theta_final)]
...
def rescale_V(V, om, V_max, om_max):
    ...
    V_tilde = np.absolute(V_candidates[V_tilde_indexes, np.arange(len(V))])    
    ########## Code ends here ##########
    return V_tilde
...
def compute_tau(V_tilde, s):
    ...
    """
    ########## Code starts here ##########
    tau = cumtrapz(y=1.0/V_tilde, x=s, initial=0)    
    ########## Code ends here ##########
    return tau

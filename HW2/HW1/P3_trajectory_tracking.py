...
class TrajectoryTracker:
    ...
    def compute_control(self, x, y, th, t):
        ...
        # compute virtual control inputs [u1, u2]
        xd = self.V_prev*np.cos(th)
        yd = self.V_prev*np.sin(th)
        virtual_control_inputs = np.array([xdd_d + self.kpx*(x_d-x) + self.kdx*(xd_d-xd), 
        ...
        # control histories [a,om]
        control_histories = np.linalg.solve(J, virtual_control_inputs) 
        
        # compute actual control inputs V,om
        a = control_histories[0]
        V = self.V_prev + dt*a
        om = control_histories[1]
